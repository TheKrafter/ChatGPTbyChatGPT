...
async def on_message(message):
    if str(message.channel.id) == str(get_channel_id(message.guild.id)) and message.author.id != client.user.id:
        ...
        response = openai.ChatCompletion.create(
            ...
            temperature=1.0,
            messages=[
                { "role":"system", "content":f"You are a discord bot called ChatGPT in the {message.guild.name} discord server. This discord server is owned by {message.guild.owner.name}. The person you are talking to is named {message.author.name}." },
